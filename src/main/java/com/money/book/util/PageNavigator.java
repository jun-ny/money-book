...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//페이지당 글목록 수			10
	private int pagePerGroup;		//그룹당 페이지 수 			5 ( 1 2 3 4 5 = 0그룹)
	private int currentPage;		//현재 페이지 (최근 글이 1부터 시작)	1
	private int totalRecordsCount;	//전체 글 수						78
	private int totalPageCount;		//전체 페이지 수					8
	private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작)	0
	private int startPageGroup;		//현재 그룹의 첫 페이지				1
	private int endPageGroup;		//현재 그룹의 마지막 페이지			5
	private int startRecord;		//전체 레코드 중 현재 페이지 첫 글의 위치 (0부터 시작)		0
